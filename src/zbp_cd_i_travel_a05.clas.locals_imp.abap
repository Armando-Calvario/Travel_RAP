...
*    endif.
    ...
    DATA(lv_auth) = COND #( WHEN cl_abap_context_info=>get_user_technical_name(  )  EQ 'CB9980000557'
...
* Modify in local mode - BO - related updates there are not relevant for authorization objects
    MODIFY ENTITIES OF zcd_i_travel_a05 IN LOCAL MODE
           ENTITY Travel
    ...
    READ ENTITIES OF zcd_i_travel_a05 IN LOCAL MODE
         ENTITY Travel
         FIELDS ( agency_id
                  customer_id
                  begin_date
    ...
    LOOP AT lt_travel ASSIGNING FIELD-SYMBOL(<ls_travel>).
      ...
      APPEND VALUE #( travel_id = <ls_travel>-travel_id
                          %msg      = new_message( id = 'ZMC_TRAVEL_A05'
                                                   number = '005'
                                                   v1 = lv_travel_msg
...
* reported
    ...
    READ ENTITIES OF zcd_i_travel_a05
    ENTITY Travel FIELDS ( travel_id agency_id customer_id booking_fee total_price currency_code )
...
*    REPORTED reported.
    ...
    DATA lt_create_travel TYPE TABLE FOR CREATE zcd_i_travel_a05\\Travel.

    SELECT MAX( travel_id ) FROM ztb_travel_a05
...
* Modify in local mode - BO - related updates there are not relevant for authorization objects
  ...
  METHOD validateDates.
    ...
    READ ENTITY zcd_i_travel_a05\\Travel
    ...
    LOOP AT lt_travel_result INTO DATA(ls_travel_result).
      ...
      ELSEIF ls_travel_result-begin_date < cl_abap_context_info=>get_system_date(  ). "begin_date must be in the
...
CLASS lsc_ZCD_I_TRAVEL_A05 DEFINITION INHERITING FROM cl_abap_behavior_saver.
...
CLASS lsc_ZCD_I_TRAVEL_A05 IMPLEMENTATION.

  METHOD save_modified.

    DATA: lt_travel_log   TYPE STANDARD TABLE OF ztb_log_a05,
          lt_travel_log_u TYPE STANDARD TABLE OF ztb_log_a05.
    ...
    IF NOT create-travel IS INITIAL.
      ...
      LOOP AT lt_travel_log ASSIGNING FIELD-SYMBOL(<ls_travel_log>).
        ...
        <ls_travel_log>-changing_operation = lsc_zcd_i_travel_a05=>create.
    ...
    IF NOT update-travel IS INITIAL.
      ...
      LOOP AT update-travel INTO DATA(ls_update_travel).
        ...
        <ls_travel_log_bd>-changing_operation = lsc_zcd_i_travel_a05=>update.
    ...
    IF NOT delete-travel IS INITIAL.
      ...
      LOOP AT lt_travel_log ASSIGNING FIELD-SYMBOL(<ls_travel_log_del>).
        ...
        <ls_travel_log_del>-changing_operation = lsc_zcd_i_travel_a05=>delete.
    ...
    IF  NOT lt_travel_log IS INITIAL.

      INSERT ztb_log_a05 FROM TABLE @lt_travel_log_u.
